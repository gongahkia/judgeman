...
func DefaultConfig() Config {
	...
	configDir := filepath.Join(homeDir, ".config", "sato-pulse")
	...
	return Config{
		...
		Mpv: MpvConfig{
			SocketPath:    "/tmp/sato-pulse-mpv.sock",
...
func configPath(override string) string {
	...
	homeDir, _ := os.UserHomeDir()
	return filepath.Join(homeDir, ".config", "sato-pulse", "config.toml")
}
...
func mergeSharedProfiles(cfg *Config) {
	...
	if profilePath == "" {
		candidates := []string{
			filepath.Join("..", "shared", "mood_profiles.json"),  // from sato-pulse/

...
func (b *SatoPulseBridge) OnMoodChange(mood string, confidence float64, source string) (bool, error) {
	b.mu.Lock()
...
func (b *SatoPulseBridge) switchMusic(mood string, confidence float64) (string, error) {
...
func (b *SatoPulseBridge) fetchAndAppend(mood string) {
	queries, ok := b.moodQueries[mood]
	if !ok || len(queries) == 0 {
		return
	}
...
func (b *SatoPulseBridge) OnTrackEnded() {
	if b.queueMgr == nil {
...
func (b *SatoPulseBridge) OnPreviousTrack() {
	if b.queueMgr == nil {
		slog.Warn("bridge: queue unavailable")
		return
	}
...
func (b *SatoPulseBridge) SetDND(enabled bool) {
	b.mu.Lock()
	defer b.mu.Unlock()
	b.dndMode = enabled
}

func (b *SatoPulseBridge) BlacklistCurrent() error {
	if b.queueMgr == nil {
...
func (b *SatoPulseBridge) CurrentMood() string {
	b.mu.Lock()
	defer b.mu.Unlock()
	return b.currentMood
}

func (b *SatoPulseBridge) currentMoodState() (string, string) {
	b.mu.Lock()
	defer b.mu.Unlock()
	return b.currentMood, b.currentMoodEventID
}

func (b *SatoPulseBridge) filterTracks(tracks []ipc.Track) []ipc.Track {
	if b.blacklist == nil {
...
func (b *SatoPulseBridge) annotateTracks(tracks []ipc.Track, query string) []ipc.Track {
...
func (b *SatoPulseBridge) playTrack(track ipc.Track) error {
	if b.provider == nil {
...
func (b *SatoPulseBridge) Provider() PlaybackProvider {
	return b.provider
}

func (b *SatoPulseBridge) recordTrack(mood string, moodEventID string, track ipc.Track) {
	if b.analytics == nil {
		return
	}

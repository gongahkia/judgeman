...
func NewAggregator() (*Aggregator, error) {
	...
	dbPath := filepath.Join(homeDir, ".local", "share", "mood-music", "history.db")
...
func (a *Aggregator) ensureDB() error {
	...
	_, err = db.Exec(`
		...
		CREATE TABLE IF NOT EXISTS mood_music_log (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			mood_event_id TEXT,
			mood TEXT NOT NULL,
			search_query TEXT,
		...
		CREATE INDEX IF NOT EXISTS idx_mood_music_log_ts ON mood_music_log(timestamp);
	`)
	if err != nil {
		db.Close()
...
func ensureSchema(db *sql.DB) error {
	...
	if err := ensureColumn(
		...
		"mood_music_log",
		"mood_event_id",
		"ALTER TABLE mood_music_log ADD COLUMN mood_event_id TEXT",
	); err != nil {
		return err
	}
	...
	_, err := db.Exec(`
		...
		CREATE INDEX IF NOT EXISTS idx_mood_music_log_event_id ON mood_music_log(mood_event_id);
	`)
	if err != nil {
...
func (a *Aggregator) RecordTrack(moodEventID, mood, query, videoID, title, artist string) error {
	...
	_, err := a.db.Exec(
		"INSERT INTO mood_music_log (mood_event_id, mood, search_query, video_id, title, artist) VALUES (?, ?, ?, ?, ?, ?)",

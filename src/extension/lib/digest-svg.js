...
export function renderDigestSvg({
    ...
    const rows = groups.slice(0, 5).map(([target, targetEntries], index) => {
        ...
        return `<text x="86" y="${y}" font-family="Arial, Helvetica, sans-serif" font-size="20" fill="#191411">${escapeSvgText(target)}: ${targetEntries.length}</text>`;
    }).join('\n  ');
  ...
  <text x="64" y="130" font-family="Arial, Helvetica, sans-serif" font-size="16" fill="#9a4516" letter-spacing="2">WEEKLY DIGEST</text>
  <text x="64" y="162" font-family="Arial, Helvetica, sans-serif" font-size="18" fill="#66594d">${escapeSvgText(generatedDate)} / ${entries.length} saved prompt${entries.length === 1 ? '' : 's'}</text>
  ${rows || '<text x="86" y="188" font-family="Arial, Helvetica, sans-serif" font-size="20" fill="#191411">No saved prompts in the last 7 days.</text>'}
  <path d="M650 72h64v64h-64z" fill="#a14419"/>
  <path d="M666 88h32v32h-32z" fill="#fff8ea"/>
  <text x="568" y="332" font-family="Arial, Helvetica, sans-serif" font-size="18" fill="#66594d">local-only digest</text>
</svg>`;
}

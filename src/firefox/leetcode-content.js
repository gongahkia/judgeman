        ...
        const observer = new MutationObserver((mutations) => {
            ...
            for (let span of resultSpans) {
                ...
                if (span.textContent.includes('Accepted')) {
                    ...
                    console.log('Failure');
                    browser.runtime.sendMessage({ action: "submissionResult", success: false });
                    resolve('failure');
                }
                return;
            }
...
(async function() {
    ...
    const { lastSubmittedSolution, lastQuestionSlug } = await browser.storage.local.get(['lastSubmittedSolution', 'lastQuestionSlug']);

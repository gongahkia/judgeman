# Sato + Vibecheck Integration

This document covers the integration between **Sato** (collaborative Spotify blend builder) and **Vibecheck** (emotion-driven desktop music player).
...
- `GET /api/me/mood-summary` -- returns vibecheck desktop mood distribution (if available)
...
- `backend/mood_history.py` -- reads vibecheck's SQLite history for mood suggestions
...
4. Classifies into 5 moods using the same algorithm as vibecheck's Python classifier
5. Applies weighted smoothing (window=5) and debouncing (3 frames)
6. Click "Use [MOOD] as mood source" to set it

**Frontend files:**
- `sato-app/src/lib/mood-classifier.js` -- JS port of vibecheck's `classifier.py`
...
If vibecheck has been running on the same machine, Sato reads its mood history to suggest a mood:

- Reads `~/.local/share/mood-music/history.db` (vibecheck's SQLite)
...
## Feature: Spotify Playback Backend (Vibecheck)

Vibecheck can now use Spotify instead of YouTube Music + mpv for playback.

**Config** (`~/.config/mood-music/config.toml`):
```toml
[playback]
...
**Architecture:**
- `PlaybackProvider` interface in `vibecheck/internal/bridge/provider.go`
...
**PKCE Auth:** Run `mood-music configure` and set `SPOTIFY_CLIENT_ID` env var. Auth uses Authorization Code with PKCE (no client secret needed). Tokens stored at `~/.config/mood-music/spotify_tokens.json`.
...
- `vibecheck/mood-engine/mood_engine/spotify_handler.py` -- IPC handlers
- `vibecheck/internal/bridge/spotify_provider.go` -- Go-side provider
- `vibecheck/internal/bridge/ytmusic_provider.go` -- refactored existing path

## Shared Mood Profile Loading (Vibecheck)

Vibecheck's Go config now loads `shared/mood_profiles.json` and merges queries with TOML overrides:
